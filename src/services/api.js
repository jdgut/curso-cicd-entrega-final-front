...
export async function joinTrip(tripId, actor_email) {
    const response = await api.post(`/trips/${tripId}/join`, { actor_email });
    return response.data;
}
export async function leaveTrip(tripId, actor_email) {
    const response = await api.post(`/trips/${tripId}/leave`, { actor_email });
    return response.data;
}
export async function updateTripState(tripId, actor_email, new_state) {
    const response = await api.post(`/trips/${tripId}/state`, { actor_email, new_state });
    return response.data;
}
